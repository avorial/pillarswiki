---
category: organization
type: organization
name: "Nomad Packs"
summary: "Visiting and peripheral Sabbat nomad packs"
tags:
  - "reference"
  - "Sabbat"
visibility: players
approvalStatus: approved
knownToPlayers: true
keyLinks: []
aliases: []
lastEditedBy: "Codex import from pillarswiki"
---
# Nomad Packs

## Overview

Visiting and peripheral Sabbat nomad packs

- Clan / sphere: Sabbat
- Haven: Mobile, temporary, or peripheral havens depending on the pack.
- Influence: Variable nomad influence across the wider Los Angeles Sabbat theater.

## Traits

| Field | Value |
| --- | --- |
| Leader | Varies by pack |
| Nature | Nomad Packs |
| Allegiance | Sabbat |
| Primary Territory | Mobile and peripheral Los Angeles fronts |
| Known Members | 4 packs summarized |
| Purpose | To represent visiting, stranded, and peripheral Sabbat packs active around the Los Angeles war |

## Attributes

| Field | Value |
| --- | --- |
| Sphere | Vampire |
| Faction | Sabbat |
| Structure | Nomad Packs |

## Abilities

| Field | Value |
| --- | --- |
| Territory | Mobile and peripheral Los Angeles fronts |
| Members | Never Ending Disaster Clique; Angeli della Notti; Schwarze Sonne; Sunset Ravagers |
| Function | Raiding, reinforcement, peripheral pressure, and unresolved Sabbat continuity |

## Powers

| Field | Value |
| --- | --- |
| Allegiance | Sabbat |
| Role In The City | Visiting and peripheral Sabbat nomad packs |
| Storyteller Use | Use these packs as mobile complications, visiting allies, old liabilities, and reminders that the Sabbat war around Los Angeles extends beyond the city map. |

## History

These are the Sabbat nomad packs and peripheral remnants currently summarized for Pillars of Salt. Each one is kept as its own compact entry until a fuller pack dossier is written.

## Appearance

Symbol or group image pending for Nomad Packs.

## Roleplaying Notes

Treat each nomad pack as its own pressure point rather than interchangeable Sabbat muscle. Their value is in the different histories and methods they bring to Los Angeles.
